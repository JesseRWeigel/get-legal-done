# Contributing to get-legal-done

Thank you for your interest in contributing to **get-legal-done** — an AI copilot for legal research and brief writing.
...
git clone https://github.com/JesseRWeigel/get-legal-done.git
cd get-legal-done
...
Protocols live in `src/gld/specs/references/protocols/`. Each file encodes a
domain methodology that agents follow during execution.

Existing protocols:
  - `case-law-research-protocols.md`
  - `constitutional-analysis-protocols.md`
  - `statutory-interpretation-protocols.md`

To add a new one:

1. Create a Markdown file in `src/gld/specs/references/protocols/`, e.g.
   `case-law-research-protocols.md`.
...
Agents are Markdown files with YAML frontmatter in `src/gld/agents/`.
...
1. Create `src/gld/agents/gld-citation-checker.md`.
2. Include the required YAML frontmatter:

   ```markdown
   ---
   name: gld-citation-checker
   ...
   <role>
   You are the **GLD Citation Checker** — ...
   </role>
   ```
...
Verification predicates live in `src/gld/core/kernel.py`. Each check is a
function that inspects evidence and returns a `CheckResult`.

To add a new check:

1. Open `src/gld/core/kernel.py`.
2. Add a predicate function:

   ```python
   def check_citation_validity(evidence: dict[str, Any]) -> CheckResult:
       """Checks that all case citations are real and correctly cited."""
       # Inspect evidence...
       passed = ...  # your logic
       return CheckResult(
           check_id="citation_validity",
...
1. Create a Python module under `src/gld/mcp/`, e.g.
   `src/gld/mcp/citation-checker_server.py`.
2. Implement the server using the MCP SDK:

   ```python
   from mcp.server import Server

   server = Server("gld-citation-checker")
...
get-legal-done follows the **Get-Paper-Done (GPD)** architecture — a shared framework
...
**get-legal-done is a research and productivity tool, not a law firm.** It does not
provide legal advice. Outputs generated by this software:

- Are **not** a substitute for the advice of a licensed attorney.
- May contain errors, hallucinations, or outdated information.
- Must be independently verified before use in any legal proceeding.
- Do not create an attorney-client relationship.

Contributors should ensure that documentation, examples, and agent prompts include
appropriate disclaimers and never imply that the tool's outputs constitute legal advice.
