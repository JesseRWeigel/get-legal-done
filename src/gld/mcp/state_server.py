"""MCP server for GLD state management.

Exposes tools for loading, saving, and querying legal project state
...
from gld.core.state import StateEngine, ProjectState
from gld.core.constants import ProjectLayout

server = Server("gld-state")
...
async def list_tools() -> list[Tool]:
    return [
        Tool(
            name="load_state",
            description="Load the current legal project state from state.json.",
            inputSchema={
                "type": "object",
                "properties": {
                    "project_root": {
