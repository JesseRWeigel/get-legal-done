...
Read `{GLD_INSTALL_DIR}/specs/workflows/execute-phase-workflow.md` first. Do NOT improvise.
...
6. Runs post-phase verification (CRITICAL for legal work — citation verification)
7. Handles gap closure if verification fails

### Pre-execution
1. Load state via gld-state
2. Check conventions via gld-conventions (jurisdiction and citation format MUST be set)
...
2. For each plan/task in the wave (parallel):
   a. Spawn gld-executor subagent with task context
...
4. Run inter-wave verification if configured (citation spot-checks)
5. Update STATE.md

### Post-Phase Verification
After all waves complete:
1. Spawn gld-verifier with all phase artifacts (THIS IS NON-NEGOTIABLE)
...
- Citation verification failure: REMOVE hallucinated citations, replace with verified ones
